...
def personel_tebellug(event):
    ...
    if liste3=="":
        ...
        if not liste3 in tebellug:
            tebellug.append(liste3)
            ...
            img=PhotoImage(file='teblig.png')
            uyari.tk.call('wm','iconphoto',uyari._w,img)
            Label(uyari, text ='Personel Seçili!').pack()
...
def cikti():
    tebellug1=sorted(tebellug, key=locale.strxfrm)
    ...
    while True:
        for s in range(0,len(tebellug1)):
            
            vt4= sqlite3.connect(tebellug1[s]+'.s3')
            im4= vt4.cursor()
    ...
    if tebellug1==[] or personel_olan==[] or kaymakamlik1=="" or okul_adi1=="" or teblig_eden1=="" or teblig_eden_gorev1=="" or teblig_edilen_yazi_tarih1=="" or teblig_edilen_yazi_sayisi1=="" or teblig_edilen_yazi_konusu1=="" or teblig_yeri1=="" or teblig_tarih1=="" or teblig_saat1=="" :
        uyari=Toplevel()
        uyari.resizable(width=FALSE ,height=FALSE)
        img=PhotoImage(file='teblig.png')
        uyari.tk.call('wm','iconphoto',uyari._w,img)
        Label(uyari, text ='Tebliğ Edilen Yazı Bilgilerini Eksiksiz Giriniz! veya\nTebellüğ Edenleri Seçiniz! veya\n Personel Bilgilerini Girip Kaydediniz!').pack()
        ...
        table = document.add_table(rows=len(tebellug1), cols=4,style = 'Table Grid')
        table.columns[0].width = Cm(0.9)
        table.columns[1].width = Cm(5.9)
        table.columns[2].width = Cm(5.9)
        table.columns[3].width = Cm(5.9)
        ...
        root.destroy()
        os.startfile("tebligvetebellug.exe")
...
def sil_tebellug():
    ...
    tebellug.remove(data_sil)

    os.remove(data_sil+".s3")
...
ttk.Label(mainframe, text ='Tebellüğ için seçip fare sağ butonu çift tıklayın.').grid(column = 7, row = 31)
...
ttk.Label(mainframe, text ='TEBELLÜĞ EDENLER LİSTESİ').grid(column = 5, row=0)
...
liste2.bind("<Double-Button-3>",personel_tebellug)
...
ttk.Button(mainframe, text='Sil', command= sil_tebellug).grid(column=5, row=32)
